-- A nested SECURITY DEFINER helper does not retain the outer function's
-- effective request identity on all PostgREST execution paths. Check the
-- entitlement directly against the JWT user captured at the start instead.
