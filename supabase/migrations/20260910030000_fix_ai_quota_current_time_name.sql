-- current_time is a PostgreSQL special value (time with time zone), so it
-- cannot safely be used as a PL/pgSQL variable name in timestamp comparisons.
...
begin
  ...
  if not exists (
    ...
    where user_id = current_user_id
      and is_active
      and (expires_at is null or expires_at > request_time)
  ) then
  ...
  if exists (
    ...
    where user_id = current_user_id
      and status = 'reserved'
      and requested_at > request_time - interval '5 minutes'
  ) then
  ...
  if exists (
    ...
    where user_id = current_user_id
      ...
      and requested_at > request_time - interval '60 seconds'
  ) then
  ...
  where user_id = current_user_id
    and requested_at > request_time - interval '1 minute';
  select count(*) into daily_count from public.ai_request_usage
  where user_id = current_user_id
    and requested_at >= date_trunc('day', request_time);
  select count(*) into monthly_count from public.ai_request_usage
  where user_id = current_user_id
    and requested_at >= date_trunc('month', request_time);
  ...
  insert into public.ai_request_usage (
    ...
    current_user_id, p_request_hash, p_input_characters, request_time
