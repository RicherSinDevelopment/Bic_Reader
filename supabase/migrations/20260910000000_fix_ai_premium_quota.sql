-- reserve_ai_request is SECURITY DEFINER. Calling the public SECURITY INVOKER
-- wrapper from it evaluates the wrapper as the function owner, so a valid JWT
-- customer can be incorrectly denied as non-premium. Use the private helper,
-- which reads auth.uid() from the request JWT, directly.
