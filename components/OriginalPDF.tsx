...
let baseRatio=1;function buildPlaceholders(count,ratio){baseRatio=ratio;const fragment=document.createDocumentFragment();for(let n=1;n<=count;n++){const section=document.createElement('section');section.className='page';section.dataset.page=String(n);section.style.height=(document.documentElement.clientWidth*ratio)+'px';fragment.appendChild(section)}pages.appendChild(fragment)}
...
// Orientation / frame changes alter every page's width, so re-flow all of the
// unrendered placeholders and re-render the visible page at the new size,
// preserving the current page's on-screen position. Without this the vertical
// scroll position and page heights stay at the pre-rotation size and clip.
let reflowTimer;window.addEventListener('resize',()=>{clearTimeout(reflowTimer);reflowTimer=setTimeout(()=>{const width=document.documentElement.clientWidth,page=pageAtViewportCenter(),section=pages.children[page-1],prevTop=section?.offsetTop||0,prevScrollY=window.scrollY;for(const sec of pages.children){const number=Number(sec.dataset.page);if(!rendered.has(number))sec.style.height=(width*baseRatio)+'px'}renderPage(page,1,true);const nextSection=pages.children[page-1];if(nextSection){const delta=nextSection.offsetTop-prevTop;window.scrollTo({top:Math.max(0,prevScrollY+delta),left:0,behavior:'auto'})}},80)});
