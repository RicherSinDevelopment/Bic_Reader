...
<style>
*{box-sizing:border-box}html,body{margin:0;background:#f7f5ec;color:#222;font-family:-apple-system,BlinkMacSystemFont,sans-serif}body{overflow-y:auto;-webkit-overflow-scrolling:touch}#pages{padding:8px 0 48px}.page{position:relative;width:100%;margin:0 auto 8px;background:white;box-shadow:0 1px 2px #0002;overflow:hidden;contain:layout paint}.page canvas{display:block;width:100%;height:100%}.textLayer{position:absolute;inset:0;overflow:hidden;opacity:1;line-height:1;text-align:initial;text-size-adjust:none;transform-origin:0 0;z-index:3}.textLayer :is(span,br){position:absolute;color:transparent;white-space:pre;cursor:text;transform-origin:0 0}.textLayer ::selection{background:rgba(40,120,255,.35)}.switchHighlight{position:absolute;z-index:2;border-radius:3px;background:rgba(255,210,42,.62);box-shadow:0 0 0 1px rgba(214,160,0,.18);pointer-events:none}.status{position:fixed;inset:0;display:flex;align-items:center;justify-content:center;background:#f7f5ec;z-index:20;color:#555}.spinner{width:28px;height:28px;border:3px solid #8fb99644;border-top-color:#8fb996;border-radius:50%;animation:spin .8s linear infinite;margin-right:12px}@keyframes spin{to{transform:rotate(360deg)}}
...
async function goToPage(page,smooth=true){const number=Math.max(1,Math.min(totalPages||page,Number(page)||1)),section=pages.children[number-1];if(!section)return;suppressScroll=true;currentPage=number;await renderPage(number);section.scrollIntoView({behavior:smooth?'smooth':'auto',block:'start'});unloadFarPages(number);send({type:'pageChanged',page:number,totalPages});setTimeout(()=>{suppressScroll=false},smooth?700:100)}
