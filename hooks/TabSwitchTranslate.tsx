...
export function readerDestinationForTranslatedAnchor(
  ...
  const sourceBlock = readerBlocks.find(
...
export function useTabSwitchTranslate(input: {
  ...
  const translatedPublishTimer = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    if (translatedPublishTimer.current)
      clearTimeout(translatedPublishTimer.current);
  ...
  const reportReaderAnchor = useCallback(
    (blockId: string, word: string, wordIndex: number) => {
      if (!input.documentId || activeMode.current !== "reader") return;
      if (input.isRotationInProgress?.()) {
        if (readerPublishTimer.current)
          clearTimeout(readerPublishTimer.current);
        readerPublishTimer.current = null;
        return;
      }
    ...
    [
      input.documentId,
      input.isRotationInProgress,
      input.reportVisibleReaderBlock,
    ],
  );

  const reportTranslatedAnchor = useCallback(
    (blockId: string, _word: string, wordIndex: number) => {
      ...
      const block = latestTranslatedBlocks.current.find(
        (candidate) => candidate.id === blockId,
      );
      if (!block) return;
      const words = wordsIn(block);
      ...
      if (
        current?.documentId === input.documentId &&
        current.sourceBlockId === sourceId &&
        Math.abs((current.blockProgress ?? 0) - progress) < 0.015
      )
        return;
      if (translatedPublishTimer.current)
    ...
    [input.documentId, input.isRotationInProgress, input.languageCode],
  );
